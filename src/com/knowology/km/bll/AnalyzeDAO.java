...
//	}
	 ...
	 * @param queryCityCode
	 *            问题地市
	 * @return success : 生成结果 如果生成成功，结果中wordpat为词模<br>
	...
	public static List<String> getAllQuery(String serviceid, int querytype) {
		...
		if (rs != null && rs.getRowCount() > 0) {
			...
			for (int i = 0; i < rs.getRowCount(); i++) {
				...
				String isserviceword = rs.getRows()[i].get("isstrictexclusion") == null ? " "
						: rs.getRows()[i].get("isstrictexclusion").toString();
...
//			jsonObj.put("wordpatList", StringUtils.join(wordpatResultList, "@_@"));
	...
	public static void main(String[] args) {
		// String worpattosimworpat =
		// SimpleString.worpattosimworpat("测试近类@2#编者=\"你好\"");
		// worpattosimworpat = SimpleString.worpattosimworpat("测试近类@2# ");
		// System.out.println(worpattosimworpat);

		Map<String, String> map = new HashMap<String, String>();
		// String segments = "我(!本人近类|!本行近类|!第一人称近类|!集团无效咨询单字父类|!冗余词词类|!我近类|模板词)
		// 想(!集团无效咨询单字父类|!考虑近类|!梦想近类|!想近类|!想念近类|模板词)
		// 去(!到近类|!火车到近类|!集团无效咨询单字父类|!去近类|!往近类|!走近类|模板词)
		// 北京(!北京近类|!航班城市父类|!嘉实银行父类|!原子词词类|!中国城市父类|!中国行政城市父类|模板词) 天安(!原子词词类|模板词)
		// 门(OOV) 看看(!电信业务父类|!看近类|模板词) (7 words)";
