...
public class UnderstandMain {
	
	public final static String MSG = "调用方式：java -jar ProduceMain userid serviceid";
	
	public static List<QueryUnderstander> producerPool = new ArrayList<QueryUnderstander>();
		...
		if (rs != null && rs.getRowCount() > 0){
			for(SortedMap row : rs.getRows()){
				...
				if(key != null){
					...
					NLP_SERVER_IP_MAP.put(value, "http://" + key + "/NLPAppWS/AnalyzeEnterPort?wsdl");
//					NLP_SERVER_IP_MAP.put(value, "http://" + key + "/NLPWebService/NLPCallerWS?wsdl");
	...
	public static boolean allStoped(){
		for(QueryUnderstander producer : producerPool){
	...
	public static void printStatistics(){
		...
		for(QueryUnderstander producer : producerPool){
			System.out.println("线程[" + producer.getName() + "]成功理解" + producer.getCounter() + "个");
		}
	}
	...
	public static void main(String[] args) {
		InputStream in = UnderstandMain.class.getResourceAsStream("config.properties");
...
//		}
		...
		for(int i = 0; i < threadNum; i++){
			QueryUnderstander producer = new QueryUnderstander(taskQueue);
		...
		while(true){
			...
			if(allStoped()){
				...
				System.out.println("批量理解完成，总成功" + countSuccess() + "个。共耗时：" + (endTime2 - startTime2)/1000 +"秒");
				printStatistics();
				return;
			}
		}
